...
if __name__ == "__main__":
    Clear()
    print(Colorate.Horizontal(Colors.red_to_purple, f"made with <3 by yzde "))
    itemID = Write.Input("type the video link here > ", Colors.red_to_purple, interval=0.0001)
    amount = Write.Input("Amount of shares? (Remember that 0 equals infinite!) > ", Colors.red_to_purple, interval=0.0001)
    NThread = Write.Input("Thread Amount > ", Colors.red_to_purple, interval=0.0001)

    if Title("proxy generator"):
